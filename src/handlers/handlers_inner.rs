...
pub async fn delete_question(
    ...
    let result = questions_dao
        .delete_question(question_uuid.question_uuid.to_string())
        .await;
...
pub async fn create_answer(
    ...
    match answer {
        ...
        Err(err) => {
            ...
            match err {
                DBError::InvalidUUID(s) => Err(HandlerError::BadRequest(s.to_string())), // return a `HandlerError::BadRequest` error passing in s as the string
...
pub async fn read_answers(
    ...
    match answers {
        ...
        Err(e) => {
            error!("Error creating answers: {:?}", e);
            Err(HandlerError::default_internal_error()) // return a default internal error using the HandlerError type
        }
    }
}
...
pub async fn delete_answer(
    ...
    let result = answers_dao.delete_answer(answer_uuid.answer_uuid).await; // delete answer using `answers_dao`
